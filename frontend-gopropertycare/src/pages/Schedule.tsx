...
export function Schedule() {
  ...
  // Read URL query parameters for pre-selected tier & sqft
  useEffect(() => {
    ...
    if (sqftParam) {
      const parsedSqft = parseInt(sqftParam, 10);
      if (!isNaN(parsedSqft) && parsedSqft >= 100) {
        setSqft(parsedSqft);
      }
    }

    if (tierParam && rates.length > 0) {
      const match = rates.find((r) => r.service_type === tierParam);
      if (match) {
        setSelectedRateId(match.id);
      }
    }
  }, [location.search, rates]);
    ...
    const fetchApiData = async () => {
      try {
        ...
        if (ratesRes?.data) {
          ...
          if (list.length > 0) {
            setRates(list);
            setSelectedRateId((prev) => prev || list[0].id);
          }
        }
  ...
  return (
    <div className="max-w-4xl mx-auto py-4 sm:py-8 space-y-8 animate-fade-in">
      ...
      <div className="text-center space-y-3">
        ...
        {/* Steps Bar */}
        <div className="grid grid-cols-4 gap-2 sm:gap-4 max-w-xl mx-auto pt-4">
          {[1, 2, 3, 4].map((s) => {
            ...
            return (
              <div key={s} className="flex flex-col items-center gap-1.5">
                ...
                <span
                  className={`text-[11px] font-bold truncate max-w-[80px] sm:max-w-none ${
      ...
      {step === 1 && (
        <Card className="p-6 sm:p-9 space-y-8 animate-fade-in">
          ...
          <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-6 space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 bg-white border border-slate-300 rounded-xl px-4 py-2 shadow-2xs">
                ...
                />
                <span className="text-slate-500 font-bold text-sm">sq ft</span>
              </div>
            </div>
            ...
            {isMinApplied && (
              <div className="p-3 bg-emerald-50/80 border border-emerald-200 text-emerald-800 text-xs rounded-xl flex items-center gap-2">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pt-3 mt-3 border-t border-slate-100 flex items-center justify-between">
                      <span className="text-xs text-slate-400 font-medium">Estimated Base</span>
      ...
      {step === 2 && (
        <Card className="p-6 sm:p-9 space-y-8 animate-fade-in">
          <div className="border-b border-slate-100 pb-4">
            ...
            <p className="text-xs text-emerald-800 bg-emerald-50 px-3 py-1.5 rounded-lg inline-block mt-2 font-semibold">
              {t('schedule.dateNotice')}
            </p>
          </div>

          {/* Month Navigation & Calendar */}
          <div className="max-w-md mx-auto bg-white border border-slate-200 rounded-2xl p-5 shadow-2xs">
          ...
          <div className="space-y-3 max-w-md mx-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div className="text-[11px] text-slate-500 mt-1">8:00 AM – 12:00 PM</div>
              </button>
              ...
              >
                ...
                <div className="text-[11px] text-slate-500 mt-1">1:00 PM – 5:00 PM</div>
              </button>
            </div>
          </div>
        ...
        <Card className="p-6 sm:p-9 space-y-8 animate-fade-in">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 text-xs">
                      <div className="flex items-center justify-between">
                        <span className="font-extrabold text-slate-900">{addon.name}</span>
                        <span className="font-black text-emerald-700">+${Number(addon.price).toFixed(2)}</span>
                      </div>
          ...
          <div className="space-y-1.5 pt-2">
            ...
            <textarea
              ...
              className="w-full bg-white border border-slate-300 rounded-xl p-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-500/10 shadow-2xs"
            />
          </div>
      ...
      {step === 4 && (
        <Card className="p-6 sm:p-9 space-y-8 animate-fade-in">
          ...
          <div className="bg-emerald-50/50 border border-emerald-200 rounded-xl p-6 space-y-3 text-sm">
            ...
            <div className="pt-3 border-t border-emerald-200 flex justify-between items-baseline">
              <span className="font-extrabold text-slate-900 text-base">{t('schedule.totalDue')}:</span>
              <span className="font-black text-2xl text-emerald-800">${totalPrice.toFixed(2)}</span>
            </div>
          </div>

          {/* Terms Agreement Checkbox */}
          <div className="flex items-start gap-3 bg-white p-4 border border-slate-200 rounded-xl">
            <input
              type="checkbox"
              id="agree-terms"
            ...
            />
            <label htmlFor="agree-terms" className="text-xs text-slate-600 cursor-pointer font-medium leading-relaxed">
