...
- **Infraestructura de Producción**: Totalmente preparada y automatizada para **Render** (PostgreSQL gestionado + Web Service Django con Gunicorn/WhiteNoise + Static Site React).
...
│   │   ├── core/              # Modelos base abstractos (TimeStampedModel) y HealthCheckView
...
│   ├── src/
│   │   ├── api/               # Cliente Axios dinámico con interceptores JWT y auto-refresh
...
- **Endpoints de Órdenes y Servicios** (`/api/v1/`):
  ...
  - `POST /orders/<id>/cancel/`: Cancelación de órdenes pendientes.
...
3. **Frontend Static Site (`laundrygo-web`)**:
   ...
   - Definido con `type: web` y `runtime: static`.
   - Variable `VITE_API_URL` conectada al host de `laundrygo-api`.
